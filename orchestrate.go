...
func Fan(ctx context.Context, fns ...func(context.Context) (*Stream, error)) *Stream {
...
func Race(ctx context.Context, fns ...func(context.Context) (*Stream, error)) (string, Usage, error) {
...
func Sequence(ctx context.Context, fns ...func(ctx context.Context, input string) (*Stream, error)) (*Stream, error) {
	if len(fns) == 0 {
		return StreamFromSlice(nil), nil
	}
	...
	for i, fn := range fns {
		...
		text, err := CollectText(ctx, stream)
		if err != nil {
			return nil, err
		}
		input = text
